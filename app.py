...
from prompts import BASE_SYSTEM_PROMPT, MATCH_PROMPT

app = Flask(__name__)
app.secret_key = os.getenv("SECRET_KEY", "alsos_demo_secret")

MAX_JD_PER_RUN = int(os.getenv("MAX_JD_PER_RUN", "10"))
...
def index():
    if request.method == "POST":
        ...
        if resume_text and ranked:
            top_jd = ranked[0]
            try:
                msg = MATCH_PROMPT.format(
                    resume=resume_text[:12000], jd=(top_jd.title + "\n" + top_jd.desc)[:8000]
                )
                match_analysis = chat(
